...
class WikimediaPageViewServiceTest extends TestCase {
	...
	public function testGetPageData() {
		...
		$status = $service->getPageData( [ Title::makeTitle( NS_MAIN, 'Foo' ),
			Title::makeTitle( NS_MAIN, 'Bar' ) ], 5 );
		if ( !$status->isGood() ) {
		...
		$status = $service->getPageData( [ Title::makeTitle( NS_MAIN, 'A' ),
			Title::makeTitle( NS_MAIN, 'B' ), Title::makeTitle( NS_MAIN, 'C' ) ], 1 );
		...
		$status = $service->getPageData( [ Title::makeTitle( NS_MAIN, 'A' ), Title::makeTitle( NS_MAIN, 'B' ) ], 1 );
